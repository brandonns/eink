...
# Authenticate to qBittorrent client
try:
...
# Function to draw weather data on the image
def draw_weather_data(draw, weather_image, temp, feels_like, high, low, weather_description):
    weather_image.paste(weather_image)

    if temp == feels_like:
        draw.text((55, 23), f"It's currently {temp}°F outside", font=font1, fill=text_color, anchor='lm')
    else:
        draw.text((55, 23), f"It's currently {temp}°F but feels like {feels_like}°F outside", font=font1,
    ...
    if visibility >= 10000:
        ...
        draw.text((200, 60), f"{visibility}m", font=font2, fill=text_color, anchor='lm')
...
# Main code for display and weather handling
try:
    epd = epd7in5_V2.EPD()
    epd.init()
    epd.Clear()
    ...
    # Draw weather data
    draw_weather_data(draw, weather_image, temp, feels_like, high, low, weather_description)

    # Handle jacket image if cold
    ...
    # Handle wind direction and wind speed image
    wind_directions = [
        (range(0, 10), 'N', 'wi-direction-down.bmp'),
        (range(10, 30), 'NNE', 'wi-direction-down-left.bmp'),
        (range(30, 60), 'NE', 'wi-direction-down-left.bmp'),
        (range(60, 80), 'ENE', 'wi-direction-down-left.bmp'),
        (range(80, 100), 'E', 'wi-direction-left.bmp'),
        (range(100, 120), 'ESE', 'wi-direction-up-left.bmp'),
        (range(120, 150), 'SE', 'wi-direction-up-left.bmp'),
        (range(150, 170), 'SSE', 'wi-direction-up-left.bmp'),
        (range(170, 190), 'S', 'wi-direction-up.bmp'),
        (range(190, 210), 'SSW', 'wi-direction-up-right.bmp'),
        (range(210, 240), 'SW', 'wi-direction-up-right.bmp'),
        (range(240, 260), 'WSW', 'wi-direction-up-right.bmp'),
        (range(260, 280), 'W', 'wi-direction-right.bmp'),
        (range(280, 300), 'WNW', 'wi-direction-down-right.bmp'),
        (range(300, 330), 'NW', 'wi-direction-down-right.bmp'),
        (range(330, 350), 'NNW', 'wi-direction-down-right.bmp'),
        (range(350, 361), 'N', 'wi-direction-down.bmp')
    ]
    ...
    def draw_torrent_info(torrent, index, tor_index_size, draw, font1, text_color, torrent_startx, torrent_starty,
        ...
        draw.text((torrent_startx, torrent_starty + (index * tor_index_size)), torrent.name[:28] + '...', font=font1,
                  fill=text_color)
    ...
    for torrent in qbt_client.torrents_info(sort='state', reverse=False):
        draw_torrent_info(torrent, index, tor_index_size, draw, font1, text_color, torrent_startx, torrent_starty,
                          torrent_bar_startx, torrent_bar_starty, torrent_state_startx, torrent_state_starty)
        index += 1
        if index == 5:
            break
