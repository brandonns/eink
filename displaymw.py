    ...
    if weather_status in weather_images:
        ...
        for key, image in primary_weather_images.items():
            if weather_status.startswith(key):
                ...
                break
    waifu_image.paste(weather_image)

    if temp == feels_like:
        draw.text((55, 23), 'It\'s currently ' + temp + '°F outside', font=font1, fill=text_color, anchor='lm')
    else:
        draw.text((55, 23), 'It\'s currently ' + temp + '°F but it feels like ' + feels_like + '°F outside', font=font1, fill=text_color, anchor='lm')
